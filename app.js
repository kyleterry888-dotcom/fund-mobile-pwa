(() => {
  ...
  function getDisplayMode(fund) {
    ...
    if (primary.official) {
      return {
        ...
        pct: primary.pct,
        sub: `${pct(primary.pct)} · ${fund.unitNavDate || '--'}`,
        badge: '净值已出',
        source: 'official',
      };
    }
    if (primary.officialLagged) {
      ...
      return {
        ...
        value: nav(primary.nav),
        pct: primary.pct,
        sub: `${pct(primary.pct)} · 净值日 ${officialStatus.navDate || '--'} · ${officialStatus.publishDate || '--'}更新 · ${lag}`,
        badge: '海外净值已更新',
    ...
    if (primary.staleOfficialOnly) {
      ...
      return {
        ...
        value: nav(primary.nav),
        pct: null,
        sub: officialStatus.showLagWarning ? `净值日 ${officialStatus.navDate || '--'} · ${lag}` : `净值日 ${officialStatus.navDate || '--'} · 暂无盘中估值`,
    ...
    return {
      ...
      value: nav(primary.nav),
      pct: primary.pct,
      sub: `${pct(primary.pct)} · ${primary.source}${staleText}`,
  ...
  function renderFundCard(fund) {
    ...
    const holdBadge = activeHoldings.length
      ? `${manual ? '手动重仓' : '自动重仓'} · 覆盖 ${coverageText} · ${model.validCount}/${activeHoldings.length} 有行情`
      : '暂无重仓数据';
    ...
    const navFreshnessText = pos.officialReady
      ? '已覆盖为官方净值'
      : pos.officialLagged
        ? `海外滞后类：今天更新 ${navStatus.navDate || '--'} 净值`
        : navStatus.showLagWarning
          ? `海外净值日 ${navStatus.navDate || '--'} 未覆盖估值日 ${navStatus.targetDate || '--'}`
          : `估值优先 · 上次净值 ${pct(fund.dailyGrowth)} · ${fund.unitNavDate || '--'}`;
    return `
      <article class="fund-card ${expanded ? 'is-expanded' : ''}" data-code="${escapeHtml(fund.code)}">
        <div class="fund-compact">
          <div class="fund-mainline">
            <div class="fund-title compact-fund-title">
              <h3>${escapeHtml(fund.name || `基金(${fund.code})`)} ${officialBadge}</h3>
          ...
          <div class="compact-metrics">
            ...
            ${metric('持仓金额', money(pos.marketValue), '', `${todayLabel} ${money(pos.todayProfit)}`)}
            ${metric('持有收益', money(pos.totalProfit), clsByNum(pos.totalProfit), pct(pos.totalProfitPct))}
          </div>
          <div class="compact-foot">
            <span>${escapeHtml(holdBadge)}</span>
            <span>${escapeHtml(navFreshnessText)}</span>
          </div>
        </div>
